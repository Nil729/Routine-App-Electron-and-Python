...
function start(){
  ...
  if (JSON.stringify(nw1) === JSON.stringify(t2)){
    ...
    //let pyshell = new PythonShell('Open_Web.py', { scriptPath: '\extraResources' });
    ...
    PythonShell.run(Open_App, options, function (err, results) {
      if (err) throw err;
      // Results is an array consisting of messages collected during execution.
      console.log('results: %j', results);
      alert("Temps acavat") 
...
function OnAll(){
  ...
  const path = require('path');
  let OpenAll = path.join(__dirname, '/extraResources/OpenAll.py');
  ...
  PythonShell.run(OpenAll, options, function(err, results) {
    if (err) throw err;

    console.log('OpenAll.py finished.');
    console.log('OpenAll.py', results);
  });
}